...
export function ClientPortalServices() {
    ...
    const columns: Column<ClientPortalService>[] = [
        ...
        { key: 'invoiceNo', header: 'Billing', cell: (row) => <div><p className="font-bold">{row.invoiceNo || '-'}</p><p className="text-xs text-gray-500">{money(row.paidAmount)} paid / {money(row.balanceAmount)} due</p></div> },
    ...
    return (
        <div className="space-y-5">
            ...
            <Modal isOpen={Boolean(billing)} onClose={() => setBilling(null)} title="Invoice & Receipt">
                {billing && (
                    <div className="space-y-4">
                        <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                            <p className="text-[11px] font-black uppercase tracking-wide text-emerald-700">Billing Summary</p>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                            <DetailBox label="Client" value={billing.clientName} />
                            <DetailBox label="Patient" value={billing.patientName || billing.clientName} />
                            <DetailBox label="Invoice No" value={billing.invoiceNo || '-'} />
                            <DetailBox label="Invoice Amount" value={money(billing.invoiceAmount)} />
                            <DetailBox label="Receipt No" value={billing.receiptNo || '-'} />
                            <DetailBox label="Paid Amount" value={money(billing.paidAmount)} />
                            <DetailBox label="Due Amount" value={money(billing.balanceAmount)} />
                            <DetailBox label="Payment Mode" value={billing.paymentMode || '-'} />
                            <DetailBox label="Payment Status" value={<StatusHighlighter value={billing.paymentStatus || 'Pending'} />} />
                            <DetailBox label="Receipt Status" value={billing.receiptNo ? 'Available' : 'Not available'} />
                            <DetailBox label="Payment Closed On" value={formatDateTime(billing.workflowClosedAt)} />
                            <DetailBox label="Service Completed On" value={formatDateTime(billing.completedAt)} />
                        </div>

                        <div className="rounded-lg border border-amber-100 bg-amber-50 p-3 text-xs font-bold text-amber-800">
                            Download or print can be added in the next step once the final invoice format is confirmed.
                        </div>
