...
export const taskService = {
    getTasks: async (params?: { scope?: 'approval' | 'mine'; assigneeId?: string; assignedStaffId?: string }): Promise<Task[]> => {
    ...
    updateTaskStatus: async (id: string, status: 'ASSIGNED' | 'IN_PROGRESS' | 'COMPLETED' | 'APPROVED' | 'REJECTED'): Promise<Task> => {
        if (isDemoTaskId(id)) {
