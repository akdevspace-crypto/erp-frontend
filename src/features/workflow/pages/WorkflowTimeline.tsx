...
const getWorkflowExceptions = (workflow: WorkflowTimelineRecord): WorkflowException[] => {
    ...
    const duty = getStage(workflow, 'duty');
    ...
    if (!admission?.complete && !allocation?.complete) {
        addException('conversion', 'Enquiry not converted', 'Enquiry is created but admission or allocation is not completed.', 'high');
    }
    if (allocation?.complete && !duty?.complete) {
        addException('staff-duty', 'Staff duty pending', 'Allocation exists but the staff duty is not completed yet.', 'medium');
    }
    if (duty?.complete && !invoice?.complete) {
        addException('invoice', 'Invoice pending', 'Duty is completed but the finance invoice is not posted yet.', 'medium');
    }
    if (invoice?.complete && Number(workflow.summary.balanceAmount || 0) > 0) {
        addException('payment', 'Payment pending', 'Invoice is posted but full payment is not collected.', 'high');
    }
    if (payment?.complete && !closure?.complete) {
        addException('closure', 'Workflow closure pending', 'Payment is collected but workflow is not closed yet.', 'medium');
    }
    if (closure?.complete && !getStage(workflow, 'renewal-follow-up')?.complete) {
        addException('renewal', 'Renewal follow-up pending', 'Customer closure is complete but renewal follow-up has not been created.', 'low');
    }
...
export function WorkflowTimeline() {
    ...
    const totals = useMemo(() => ({
        ...
        draftInvoices: timelines.filter((item) => item.stages.some((stage) => stage.key === 'invoice' && stage.status === 'CREATED')).length,
